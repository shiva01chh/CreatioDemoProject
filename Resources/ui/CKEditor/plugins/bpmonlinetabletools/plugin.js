...
( function() {
	...
	function isEmpty(i, m) {
		return (i === null) || (i === undefined) || (!m ? i === "" : false) || (Array.isArray(i) && i.length === 0)
	}
	...
	function unmergeCells(selection) {
		const cells = getSelectedCells( selection );
		const table = cells[0].getAscendant( 'table' );
		const map = CKEDITOR.tools.buildTableMap( table );

		cells.forEach(cell => {
			const rows = parseInt( cell.getAttribute( 'rowspan' ), 10 ) || 1;
			const cols = parseInt( cell.getAttribute( 'colspan' ), 10 ) || 1;
			cell.removeAttribute( 'rowspan' );
			cell.removeAttribute( 'colspan' );
			let colIndex = getCellColIndex(cell);
			const startRowIndex = cell.getParent().$.rowIndex;
			for (let r = 1; r < rows; r++) {
				const nextRow = map[startRowIndex + r]
				const cellIndex = colIndex + cols >= nextRow.length ? colIndex - 1  : colIndex + cols;
				const nextRowEl = nextRow[cellIndex].parentNode;
				
				for (let c = 0; c < cols; c++) {
					var newCell = cell.clone();
					if (colIndex === 0 ) {
						nextRowEl.insertBefore(newCell.$, nextRow[cellIndex]);
					} else if (colIndex === nextRow.length - 1) {
						nextRowEl.appendChild(newCell.$);
					} else {
						const cellBeforeInsert = nextRow[cellIndex].nextSibling || nextRow[cellIndex];
						nextRowEl.insertBefore(newCell.$, cellBeforeInsert);
					}
				}
			}
