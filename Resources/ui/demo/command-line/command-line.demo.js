		...
		"render": function(renderTo) {
			Ext.onReady(function() {
				...
				for (var i = 2, c = settlements.length + 1; i < c; i += 1) {
					collection.add('key' + i*2, {name: settlements[i], id: String(i*3), someProperty: String(i*13)});
					collectionCL.add('key' + i*2, {name: settlements[i], id: String(i*3), someProperty: String(i*13)});
				}
				...
				model = Ext.create("Terrasoft.ViewModel", {
					values: {
						ComboboxInputValue: '',
						ComboboxSelectionText: '',
						ComboboxList: collectionCL,
						Value: null
					},
					methods: {
						onListViewItemRender: function(listViewItem) {
							listViewItem.customHtml = '<div style="height: 150px; color: red;">' +
								listViewItem.displayValue + '</div>';
						},
						onChangeTypedValue: function(inputedValue) {
							if (inputedValue.length > 0) {
								var filteredCollection = collection.filter('name', inputedValue,
										Terrasoft.StringFilterType.CONTAIN);
								var suggestString = '';
								if (filteredCollection.collection.items.length > 0) {
									suggestString = filteredCollection.collection.items[0].name;
								}
								console.log('keyPressProcess - suggestString = ' + suggestString);
								suggestString = suggestString.substring(inputedValue.length, suggestString.length);
								model.set('ComboboxSelectionText', suggestString);
								model.get('ComboboxList').clear();
								model.get('ComboboxList').loadAll(filteredCollection.collection.items);
							}
						},
						onCommandLineExecute: function() {
							var ComboboxInputValue = this.get('ComboboxInputValue');
						},
						changeTypedValue: function(a) {
							var ComboboxInputValue = this.get('ComboboxInputValue');
							console.log(a);
						}
					}
				...
				var commandLine = window.commandLine = Ext.create('Terrasoft.CommandLine', {
					renderTo: renderTo,
					width: '450px',
					bigSize: true,
					...
					typedValue: {
						bindTo: 'ComboboxInputValue'
					},
					selectionText: {
						bindTo: 'ComboboxSelectionText'
					},
					list: {
						bindTo: 'ComboboxList'
					},
					listViewItemRender: {
						bindTo: 'onListViewItemRender'
					},
					typedValueChanged: {
						bindTo: 'onChangeTypedValue'
					},
					commandLineExecute: {
						bindTo: 'onCommandLineExecute'
					},
					changeTypedValue: {
						bindTo: 'changeTypedValue'
					},
					value: {
						bindTo: 'Value'
					}
				...
				commandLine.bind(model);

				var keyPressProcess = function(model) {
					var attributes = model.attributes;
					var inputedValue = attributes.ComboboxInputValue;
					if (inputedValue.length > 0) {
						var filteredCollection = collection.filter('name', inputedValue,
								Terrasoft.StringFilterType.CONTAIN);
						var suggestString = '';
						console.log('keyPressProcess - items.length = ' + filteredCollection.collection.items.length);
						if (filteredCollection.collection.items.length > 0) {
							suggestString = filteredCollection.collection.items[0].name;
						}
						console.log('keyPressProcess - suggestString = ' + suggestString);
			...
			//	model.on('change:ComboboxInputValue', keyPressProcess, this);
			});
		}
	};
});