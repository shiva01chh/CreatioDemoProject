...
Oktell = (function(){
  ...
  * @param openTimeout connection timeout
...
//		}
		...
		var userStates = extend({},Events,{
			...
			/**
    * Getting the state and saving the new state locally
...
//				}
    ...
    * @param newState new redirect state
    * @return {Boolean} current redirect state
    */
			...
			checkUserRedirect: function(callback){
				...
				sendOktell("getredirectrules",{}, function(data){
					if ( data.result ) {
						var rule;
						if ( data.redirectrules && data.redirectrules.length > 0 ) { // searching for the rule
			...
			answer: function(callback) {
				...
				if ( self.sipActive ) {
					...
					checkIntercomSupport = function(){
						// TODO # CC-1216 Update the Oktell.js library after the fix
				...
				server = new Server( oktellOptions.url, oktellOptions.openTimeout, oktellOptions.queryDelayMin, oktellOptions.queryDelayMax, function(url){
					...
					sendOktell('login', {
						...
						if ( data.error || data.errormsg ) {
							var errorCode = 1000;
							if ( data.error == 50093 ) {
								errorCode = 1204; // User already registered

							} else if ( data.error == 50025 ) {
								errorCode = 1214; // The number of users simultaneously working with the system is limited by the license
							} else if ( data.errormsg == "Service not available" ) {
								errorCode = 1215; // Oktell service failed to load
							...
							sendOktell('getversion', {showalloweddbstoredprocs:1}, function(data){
								...
								if ( data.result ) {
                                    ...
                                    // support for several beta versions of octells, 2.11.xxxx.xxxxx comes  instead of version 2.11.1.140905 
...
//					sequence: phone.state() == phone.states.READY ? '': null,
   ...
   * @param target  the phone number or id of the user
   * @param callback
   */
