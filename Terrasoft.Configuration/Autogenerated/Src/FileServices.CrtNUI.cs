	...
	public class FileService : BaseService, IReadOnlySessionState
	{
		...
		private Stream LoadFile(Guid entitySchemaUId, Guid fileId) {
			...
			var fileRepository = item.Name != "FeedFile"
				? ClassFactory.Get<FileRepository>(new ConstructorArgument("userConnection", UserConnection))
				: ClassFactory.Get<FeedFileRepository>(new ConstructorArgument("userConnection", UserConnection));
