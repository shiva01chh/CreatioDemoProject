namespace Terrasoft.Mail.Sender {
	...
	public class ActivityEmailSender : HtmlEmailMessageSender
	{
		...
		protected virtual Activity Send(Guid activityId, Activity sourceActivity, EmailMessage message,
			...
			try {
				Logger.Debug($"[EmailSender] Loading activity before sending, Id: {activityId}");
				...
				Logger.Debug($"[EmailSender] Updating activity status before sending to Inprogress, Id: {activityId}");
				...
				var emailMessage = message ?? CreateEmailMessage(activity);
