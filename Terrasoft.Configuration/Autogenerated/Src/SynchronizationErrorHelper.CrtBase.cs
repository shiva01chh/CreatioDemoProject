namespace Terrasoft.Configuration
{
	...
	public class SynchronizationErrorHelper: ISynchronizationErrorHelper
	{
		...
		private void FireErrorHandler(Entity errorHandler, Entity mailbox, string senderEmailAddress, bool ignoreMaxRetryCount) {
			...
			if ((currentErrorCodeId.HasValue && currentErrorCodeId.Value.Equals(errorCodeId)) ||
					(currentWarningCodeId.HasValue && currentWarningCodeId.Value.Equals(errorCodeId))) {
				return;
			}
			...
			WriteErrorCode(mailbox, errorCodeId, stopped, isWarning);
		}
		...
		public virtual void ProcessSynchronizationError(string senderEmailAddress, Exception syncExeption, bool ignoreMaxRetryCount = false) {
			...
			if (errorHandler != null) {
				FireErrorHandler(errorHandler, mailbox, senderEmailAddress, ignoreMaxRetryCount);
			}
		}
