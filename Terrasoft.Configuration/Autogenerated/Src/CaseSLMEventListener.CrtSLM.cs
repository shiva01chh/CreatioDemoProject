...
namespace Terrasoft.Configuration
{
	...
	public class CaseSLMEventListener : BaseEntityEventListener
	{
		...
		private bool ClosePreviousInterval(Guid CaseId, DateTime Date) {
			...
			if (_userConnection.GetIsFeatureEnabled("AddCaseColumnInLifecycleFilter")) {
			...
			if (previousIntervals.Count > 1 && _userConnection.GetIsFeatureEnabled("CheckAndCloseAllCaseIntervals")) {
		...
		private void UpdateSolution() {
			...
			if (_userConnection.GetIsFeatureEnabled("CaseRuleApplier")) {
				...
				if (!GetIsStatusFinal(statusId) && GetIsStatusResolved(oldStatusId)) {
					...
					if (_userConnection.GetIsFeatureEnabled("ClearSolutionDateAfterReopenCase") && !isSolutionOverdue) {
						_case.SetColumnValue("SolutionDate", nullValue);
					}
				}
				if (!GetIsStatusActive(oldStatusId)) {
		...
		private bool GetStatusIs(Guid statusId, params string[] columns) {
			var result = false;
			if (!_userConnection.GetIsFeatureEnabled("UseCaseStatusStore")) {
		...
		private bool NeedCalculateTerms() {
			if (!_userConnection.GetIsFeatureEnabled("CalculateTermOnCaseEntity")) {
				return false;
			}
		...
		public override void OnSaving(object sender, EntityBeforeEventArgs e) {
			...
			if (!_userConnection.GetIsFeatureEnabled("UseCaseInSLMOldFunc")) {
				_case = (Entity)sender;
